...
const seed = async () => {
    ...
    try {
        ...
        await connection_1.db.delete(schema_1.notifications).execute();
        await connection_1.db.delete(schema_1.post).execute();
        await connection_1.db.delete(schema_1.post_categories).execute();
        await connection_1.db.delete(schema_1.users).execute();
        console.log('inserting seed data...');
        const insertedUsers = await connection_1.db.insert(schema_1.users).values([
            {
                carnet: 'C07887',
                username: 'Hugo',
                password: 'password1',
        ...
        const insertedCategories = await connection_1.db.insert(schema_1.post_categories).values([
            { name: 'Electronicos' },
            { name: 'Juguetes' },
            { name: 'Libros' },
            { name: 'Ropa' },
        ...
        const insertedPosts = await connection_1.db.insert(schema_1.post).values([
            {
